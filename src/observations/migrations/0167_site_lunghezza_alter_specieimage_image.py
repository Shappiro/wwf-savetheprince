# Generated by Django 4.1 on 2025-02-17 15:54

from django.db import migrations, models
import observations.models
import sorl.thumbnail.fields


class Migration(migrations.Migration):

    dependencies = [
        ('observations', '0166_observationdetailimage_delete_siteview_and_more'),
    ]

    operations = [
        migrations.AddField(
            model_name='site',
            name='lunghezza',
            field=models.IntegerField(blank=True, null=True, verbose_name='Lunghezza'),
        ),
        migrations.AlterField(
            model_name='specieimage',
            name='image',
            field=sorl.thumbnail.fields.ImageField(blank=True, help_text='Immagini rappresentative della specie', null=True, upload_to=observations.models.specieimage_upload_function, verbose_name='Immagine'),
        ),
    ]
